    ...
    <>
      <div className="rounded-[28px] p-6 min-h-[calc(100vh-100px)] shadow-lg text-slate-800"
        ...
        <div>
          ...
          {activeTab === "info" && (
            <div>
              <div className="flex flex-col gap-1 mb-4">
                <h2 className="text-base font-extrabold text-slate-800">Branch Information</h2>
                <p className="text-xs text-slate-400 font-medium">Full details for this branch</p>
              </div>
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
                {[
                  ...
                  <div key={label} className="flex items-center gap-4 border-b border-slate-50 pb-3">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider w-24">{label}</span>
                    <span className="text-sm font-semibold text-slate-800">{value}</span>
                  </div>
                ))}
                <div className="flex items-center gap-4">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider w-24">Status</span>
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${branch.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                    {branch.isActive ? "Active" : "Inactive"}
                  </span>
                </div>
              </div>
          ...
          {activeTab === "sales" && (
            <div>
              <div className="flex flex-col gap-1 mb-4">
                <h2 className="text-base font-extrabold text-slate-800">Sales</h2>
                <p className="text-xs text-slate-400 font-medium">Recent sales for this branch</p>
              </div>
              {sales.length > 0 ? (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-slate-100">
                        ...
                        <th className="text-left px-4 py-3 text-xs font-bold text-slate-400 uppercase tracking-wider">Total</th>
                        <th className="text-left px-4 py-3 text-xs font-bold text-slate-400 uppercase tracking-wider">Items</th>
                      </tr>
                    </thead>
                    <tbody>
                      {sales.map((sale, idx) => (
                        <tr key={idx} className="border-b border-slate-50 hover:bg-slate-50 transition">
                          <td className="px-4 py-3 text-sm font-semibold text-slate-800">{new Date(sale.date).toLocaleDateString()}</td>
                          <td className="px-4 py-3 text-sm font-semibold text-slate-700">${sale.total}</td>
                          <td className="px-4 py-3">
                            <span className="bg-blue-50 text-blue-700 text-xs font-bold px-2 py-1 rounded-lg">{sale.itemCount || "-"}</span>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
                  <div className="text-4xl mb-3">💰</div>
                  <p className="text-slate-500 font-bold text-sm">No sales data for this branch</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === "employees" && (
            <div>
              <div className="flex flex-col gap-1 mb-4">
                <h2 className="text-base font-extrabold text-slate-800">Employees</h2>
                <p className="text-xs text-slate-400 font-medium">Staff assigned to this branch</p>
              </div>
              {employees.length > 0 ? (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                  <table className="w-full">
                    ...
                    <tbody>
                      {employees.map((emp, idx) => (
                        <tr key={idx} className="border-b border-slate-50 hover:bg-slate-50 transition">
                          <td className="px-4 py-3 text-sm font-semibold text-slate-800">{emp.name}</td>
                          <td className="px-4 py-3">
                            <span className="bg-purple-50 text-purple-700 text-xs font-bold px-2 py-1 rounded-lg">{emp.role}</span>
                          </td>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
                  <div className="text-4xl mb-3">👥</div>
                  <p className="text-slate-500 font-bold text-sm">No employees assigned to this branch</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === "performance" && (
            <div>
              <div className="flex flex-col gap-1 mb-4">
                <h2 className="text-base font-extrabold text-slate-800">Performance Metrics</h2>
                <p className="text-xs text-slate-400 font-medium">Key stats for this branch</p>
              </div>
              {performance && Object.keys(performance).length > 0 ? (
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
                  <div className="text-4xl mb-3">📊</div>
                  <p className="text-slate-500 font-bold text-sm">No performance data available</p>
                </div>
              )}
            </div>
          )}
      ...
      {showEditModal && (
        <EditBranchModal
          ...
          onSuccess={() => { fetchData(); setShowEditModal(false); }}
        />
      )}
    </>
  );
